"""
GoldHybrid AI -- main_goldhybrid.py
Gold spot (XAU) spread betting main loop. Runs 24/7 Mon-Fri.
...
DASHBOARD_URL      = "http://localhost:5043/api/update"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "goldhybrid.log", encoding="utf-8"),
    ],
)
log = logging.getLogger("GoldHybrid.Main")

# ── Internal imports ──────────────────────────────────────────────────────────

import phantom_tracker
import guinevere_news
from agent_brain_gold    import get_trading_decision, format_decision_for_display
from calendar_gold       import check_calendar, is_hard_blocked, get_calendar_context
from data_feed_gold      import (
    GoldDataFeed, GOLD_EPIC, is_market_open, get_liquidity_period,
...
import performance_gold
from performance_gold    import (
...
from pre_checks_gold     import (
    run_all_pre_checks, run_individual_pre_checks, check_kill_switch_reset,
)
from strategy_gold       import should_force_close, get_gbpusd_rate, TRAILING_STOP_POINTS
...
def _dashboard_push_ok(kind, period, price, status, http) -> None:
    ...
    if not _dash_first_ok:
        _dash_first_ok = True
        log.info("Dashboard connected -- first %s push OK | period=%s gold=$%.2f status=%s HTTP %s",
                 kind, period, price, status, http)
    else:
        log.debug("Dashboard %s push | period=%s gold=$%.2f status=%s HTTP %s",
                  kind, period, price, status, http)
...
def _dashboard_push_warn(exc) -> None:
    ...
    if now - _dash_last_warn > 60:
        log.warning("Dashboard push failing (%d so far): %s -- is dashboard_gold.py running on :5043?",
...
def _get_price(ig, feed) -> float:
    """Current Gold price in USD -- Capital.com first, yfinance fallback."""
    try:
        if ig is not None and ig.connected:
            pd_ = ig.get_price(GOLD_EPIC)
            if pd_:
...
def run_candle_tick(feed, stanley, account, ig) -> None:
    ...
    log.info("--- CANDLE TICK | %s | period=%s | GOLD=$%.2f | GBPUSD=%.4f ---",
    ...
    # Guinevere news context for Arthur's prompt (cached fetch; same read reused
    # by the post-decision confidence adjustment below).
    try:
    ...
    decision = get_trading_decision(
        ...
        news_context=news_ctx, morgan_confidence=_morgan, proposed_direction=proposed_direction,
    )
    ...
    if action == "ENTER_LONG" and not stanley.in_trade:
        ...
        try:
            ...
            try:
                # Guinevere sentiment score at signal time (System 3 Review, Change 5):
                # was never logged for Gold. Cached fetch (5-min) -- no extra API call.
                try:
                    _guin_score = guinevere_news.fetch_gold_sentiment().get("score")
                except Exception:
                    _guin_score = None
                _snap = phantom_tracker.build_snapshot(
                    ind_1d, ind_1h, ind_5m,
                    morgan_score=performance_gold.get_confidence(),
                    session=period,
    ...
    if stanley.total_trades > 0 and stanley.total_trades % 50 == 0:
        generate_milestone_review(LOG_DIR / "gold_trades.csv", stanley.total_trades // 50)
...
def _open_trade(stanley, account, ig, direction, price, period, gbpusd) -> None:
    ...
    if PAPER_TRADING_MODE:
        ...
        try:
            ig.open_position(epic=GOLD_EPIC, direction="BUY" if direction == "LONG" else "SELL",
...
def _apply_confidence_lift() -> None:
    ...
    try:
        ...
        if data.get("reset_gating"):
            ...
            prior = performance_gold.get_perf_dashboard_dict().get("confidence_score")
            performance_gold.reset_to_50(reason=reason)
...
def main() -> None:
    log.info("=" * 70)
    log.info("  GoldHybrid AI v%s", VERSION)
    log.info("  Gold Spot (XAU) Spread Betting -- Capital.com")
    ...
    # Start Morgan's individual phantom-verdict feedback poller (300s daemon).
    try:
        process_new_phantom_verdicts()
    except Exception as _exc:
        log.warning("Morgan phantom poller startup failed: %s", _exc)

    # Restore Morgan's persisted confidence from the CSV audit trail so a restart
    # continues from where it left off instead of resetting to the 50.0 baseline.
    try:
        _saved_conf = load_confidence()
        if _saved_conf is not None:
            set_confidence(_saved_conf, reason='restore')
            log.info("Morgan: confidence restored to %.1f on startup", _saved_conf)
        else:
            log.info("Morgan: no persisted confidence found -- starting at baseline 50.0")
    except Exception as _exc:
    ...
    log.info("GoldHybrid AI is running. Ctrl+C to stop.")
    log.info("Dashboard: http://localhost:5043  (start dashboard_gold.py separately)")
    ...
    while not _SHUTDOWN:
        try:
            ...
            if not is_market_open(now_utc):
                ...
                log.info("Gold market closed -- next open in %s min", mins if mins else "?")
    ...
    log.info("")
    log.info("=" * 70)
    log.info("  GoldHybrid AI -- Shutdown")
    log.info("=" * 70)
    if stanley.in_trade:
    ...
    log.info("GoldHybrid AI stopped cleanly.")
