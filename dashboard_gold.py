"""
GoldHybrid AI -- dashboard_gold.py
Two-page browser dashboard at http://localhost:5043
...
log = logging.getLogger("GoldHybrid.Dashboard")
# ALBION STANDING RULE: all log timestamps are UTC (never BST/local). See main_goldhybrid.py.
...
def compute_status_fields(s: dict) -> dict:
    """Derive flat lancelot/arthur/locked_pnl fields from dashboard state.

    Wrapped in try/except so /api/state never 500s -- returns safe defaults
    on any unexpected shape. Does NOT alter trading logic; read-only view.
    """
    try:
        ...
        in_trade = bool(s.get("in_trade")) and isinstance(trade, dict)

        has_real_decision = bool(decision.get("decision"))
        # Arthur is consulted when pre-checks passed / Arthur was reached.
        # A hard calendar block pushes panel_mode="pre_checks" with no decision
        # and no pre_checks dict -> not consulted.
        arthur_consulted = (panel_mode in ("arthur", "claude")) or has_real_decision
        ...
        if arthur_consulted and decision.get("confidence") is not None:
            try:
                arthur_confidence = int(float(decision.get("confidence")))
            except (TypeError, ValueError):
                arthur_confidence = None

        # locked_pnl -- guaranteed P&L if stop is hit, in GBP.
        # stake is stake_per_point (£/pt): points * stake = GBP already, so do
        # NOT multiply by gbpusd (matches closed-trade pnl_gbp = points*stake).
        locked_pnl = None
        if in_trade:
            try:
        ...
        return {
            ...
            "arthur_consulted":      arthur_consulted,
...
def load_trades() -> list:
    """Load all Gold trades from CSV, most recent first."""
...
<title>GoldHybrid A.I. &mdash; Gold Spot</title>
<style>
:root{
  --bg:#0d0d0d;--bg2:#141414;--bg3:#1e1e1e;--border:#2a2a2a;
  --gold:#FFD700;--green:#2ecc71;--red:#e74c3c;--amber:#f39c12;
...
.nav-btn{background:rgba(255,215,0,0.15);border:1px solid var(--gold);color:var(--gold);padding:4px 12px;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer;letter-spacing:0.3px;transition:background 0.15s;}
...
.dec-hold {background:rgba(255,215,0,0.1);color:var(--gold); border:2px solid var(--gold);}
...
.pos-row{display:flex;justify-content:flex-start;align-items:center;gap:8px;padding:2px 5px;}
.pos-row>span:first-child{min-width:120px;flex-shrink:0;}
...
<div class="header">
  <div class="header-brand">
    <div>
      <div class="logo-line"><div class="logo">GOLD<span>TRADER</span> A.I.</div><span class="hdr-version" id="hdrVersion">__VERSION_STRING__</span></div>
      <div class="subtitle">Gold Spot Spread Betting &mdash; Capital.com</div>
    </div>
  </div>
  <div class="header-price">
    <div class="hdr-price-lbl">Gold Spot (USD)</div>
...
<div id="page1" class="page-wrap">
  <div class="main" id="main-grid">
    <div style="grid-column:1/-1;color:var(--muted);padding:40px;text-align:center">Loading GoldHybrid AI...</div>
  </div>
</div>
...
<div id="page2" class="page-wrap" style="display:none;">
  <div class="p2-content">
    ...
    <div class="p2-card" id="p2-trades">
      <div class="card-title">Gold Trade History</div>
      <div style="color:var(--muted);font-size:11px;">Loading...</div>
    </div>
  </div>
</div>
...
function confirmShutdown(){
  fetch('/api/shutdown', {method:'POST'})
    .then(function(){
      document.body.innerHTML = '<div style="display:flex;height:100vh;align-items:center;justify-content:center;background:#0d0d0d;color:#FFD700;font-family:monospace;font-size:18px;">GoldHybrid AI shut down. You may close this window.</div>';
    })
...
function buildLeftCol(trend1d, trend1h, signal5m, ind1d, ind1h, ind5m, liq, updatedAt){
  ...
  return '<div class="col">' +
    ...
    '</div>' +
    '<div class="card" style="flex:1"><div class="card-title gold">Liquidity Period</div>' +
...
function renderStayOutQuality(d){
  ...
  var summary =
    '<div class="soq-summary">Last 10 decisions &nbsp; Quality: <span class="gold">' + qs + '%</span></div>' +
...
function renderNewsCard(n){
  ...
  if(!n){
    return '<div class="card" id="newsCard" style="flex-shrink:0">' + title +
      '<div style="color:var(--muted);font-size:11px;">Loading gold news...</div></div>';
  }
  var sent  = n.sentiment || 'NEUTRAL';
  var scls  = sent === 'BULLISH' ? 'bull' : sent === 'BEARISH' ? 'bear' : 'neut';
  var score = (n.score === undefined || n.score === null) ? 0 : n.score;
  ...
  return '<div class="card" id="newsCard" style="flex-shrink:0">' + title + body + cauHTML + '</div>';
}
function pollNews(){
  fetch('/api/news')
    .then(function(r){ return r.json(); })
...
function renderRightPanel(d){
  ...
  var sysHTML = '<div class="card" style="flex-shrink:0"><div class="card-title gold">System Status</div>' +
    '<div class="sys-row"><span class="sys-lbl">Mode</span><span class="gold">' + (d.mode||'--') + '</span></div>' +
  ...
  var calHTML = '<div class="card" style="flex-shrink:0"><div class="card-title gold">Guinevere -- Gold Calendar</div>' +
...
function renderPage1(d){
  ...
  if(hdrEl){ hdrEl.textContent = fmtUsd(d.gold_price_usd || 0); }
  ...
  var centreCol = '<div class="col">' +
    ...
    buildPosHTML(pos, d.gold_price_usd) +
    '</div>' +
    '</div>';
...
function renderPage2(d){
  ...
  document.getElementById('p2-trades').innerHTML =
    '<div class="card-title">Gold Trade History</div>' + tradeHTML;
}
...
def api_archie_brief():
    ...
    try:
        ...
        state = get_state()
    txt = archie_brief.build_system_brief(state, "GoldHybrid", "Gold (XAU/USD)", str(LOG_DIR))
...
@app.route("/api/news")
def api_news():
    """Guinevere gold news sentiment + soft Fed/CPI/NFP caution window."""
    try:
        data = guinevere_news.fetch_gold_sentiment() or {}
        ...
        try:
            cau_active, cau_reason = guinevere_news.get_gold_calendar_status()
        except Exception:
...
if __name__ == "__main__":
    ...
    log.info("GoldHybrid AI Dashboard starting on http://localhost:%d", PORT)
