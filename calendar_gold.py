"""
GoldHybrid AI -- calendar_gold.py  (Guinevere)
Gold economic calendar. Hard blocks around the events that move gold most:
US Non-Farm Payrolls, Federal Reserve decisions, and US CPI. Plus a recurring
gold-specific hard block for the first 15 minutes of the New York open.
...
log = logging.getLogger("GoldHybrid.Guinevere")
...
def _ny_open_block(now_utc: datetime) -> Optional[str]:
    """Recurring gold hard block: first 15 minutes of the NY open (weekdays)."""
...
def check_calendar(now_utc: Optional[datetime] = None) -> dict:
    ...
    if hard_block:
        ...
        summary = f"Calendar clear -- no gold events in next 24h. {next_ev_str}"
...
def is_hard_blocked(now_utc: Optional[datetime] = None) -> tuple:
    ...
    if result["hard_block"]:
        return True, result["block_reason"], "Gold Event", HARD_BLOCK_MINUTES
    return False, "", "", 0
...
def get_calendar_context(now_utc: Optional[datetime] = None) -> str:
    ...
    lines.append("  NOTE: Gold moves hard on USD strength, Fed decisions, inflation data and geopolitics.")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    log.info("Guinevere self-test (Gold)")
