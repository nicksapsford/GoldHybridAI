"""
GoldHybrid AI -- performance_gold.py  (Morgan)
...
log = logging.getLogger("GoldHybrid.Morgan")
...
# with a timestamp so confidence can be restored on startup and reviewed later.
...
def save_confidence(confidence, reason='tick'):
    """Append one confidence reading to CONFIDENCE_LOG. level is HIGH (>=65),
    LOW (<=35) or MEDIUM otherwise. Writes a header row if the file is new.
    ...
    try:
        ...
        write_header = not os.path.exists(CONFIDENCE_LOG) or os.path.getsize(CONFIDENCE_LOG) == 0
...
def load_confidence():
    """Return the confidence float from the last row of CONFIDENCE_LOG, or None
    if the file is missing/empty/unreadable. Logs the restored value."""
    try:
        ...
        log.info("Morgan: restored confidence %.1f from CSV (%s)", value, CONFIDENCE_LOG)
        return value
    except Exception as e:
...
def set_confidence(value, reason='update'):
    ...
    with _morgan_lock:
        ...
        # Also append to the CSV audit trail / persistence log.
...
def apply_phantom_verdict_feedback(verdict, pnl_1hr, current_confidence):
    ...
    winner). Returns (adjustment, reason)."""
    if verdict == 'NEUTRAL':
    ...
    log.info("Morgan: phantom %s pnl_1hr=%s -> confidence %+.2f (from %.1f)",
...
def process_new_phantom_verdicts(get_confidence_fn=None, set_confidence_fn=None):
    ...
    Morgan's running confidence and mark them processed. Idempotent — a second
    call is a no-op if the poller is already alive."""
...
def _compute_confidence(df: pd.DataFrame) -> dict:
    ...
    # Fold in Morgan's individual phantom-verdict confidence delta (once).
...
def generate_milestone_review(trades_log: Path, milestone_num: int) -> None:
    """Save a milestone review to logs/arthur_gold_review_XX.txt every 50 trades."""
    df = _load_trades(trades_log)
    if df.empty:
        return
    perf      = _compute_confidence(df)
    breakdown = _compute_direction_liquidity_stats(df)
    review_file = REVIEW_DIR / f"arthur_gold_review_{milestone_num:02d}.txt"
    lines = [
        "=" * 60,
        f"GoldHybrid AI -- Arthur Milestone Review #{milestone_num}",
...
if __name__ == "__main__":
    ...
    log.info("Morgan self-test (Gold)")
